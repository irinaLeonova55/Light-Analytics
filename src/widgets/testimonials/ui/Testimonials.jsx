...
const Testimonials = () => {
  const reviews = [
    ...
    },
    {
      ...
      reviewText:
        'Наконец-то мы нашли аналитику, которая не врет в цифрах. Раньше приходилось сводить данные руками в Excel, потому что другие программы часто выдавали ошибки из-за сбоев в расчетах. В Light Analytics всё считается копейка в копейку. Интерфейс понятный, а сам сервис работает как швейцарские часы.',
    },
    {
      id: 3,
      foto: reviewFoto,
      ...
      userLink: '@max_retail',
      reviewText:
        'Крутой инструмент без лишней воды. Мы смогли быстро выявить убыточные товары и перераспределить бюджет на хиты продаж. Очень радует интерфейс и скорость отклика — графики грузятся мгновенно, даже при анализе больших периодов. Видно, что разработчики сделали ставку на надежность платформы.',
    },
    {
      id: 4,
      foto: reviewFoto,
      ...
      reviewText:
        'У нас огромный ассортимент, и прошлые сервисы просто не справлялись с таким объемом данных, постоянно выдавая ошибки. Эта платформа вытягивает наши 3000+ SKU вообще без проблем. Очень понятные отчеты, четкая юнит-экономика и главное — всё работает стабильно в любой сезон распродаж.',
    },
  ];
